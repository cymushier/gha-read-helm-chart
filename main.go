...
func main() {
	...
	for _, dep := range chart.Dependencies {
		fmt.Println(fmt.Sprintf(`{dependencies_%s_version}={%s} >> $GITHUB_OUTPUT`, dep.Name, dep.Version))
		fmt.Println(fmt.Sprintf(`{dependencies_%s_repository}={%s} >> $GITHUB_OUTPUT`, dep.Name, dep.Repository))
	}
}
